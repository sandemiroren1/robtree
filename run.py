...
warm_values = {
    "banknote-authenticatione0.07": 310,
"banknote-authenticatione0.09": 370,
"banknote-authenticatione0.11": 425,
"blood-transfusione0.01": 141,
"blood-transfusione0.02": 137,
"blood-transfusione0.03": 138,
"breast-cancere0.28": 86,
"breast-cancere0.39": 129,
"breast-cancere0.45": 151,
"cylinder-bandse0.23": 66,
"cylinder-bandse0.28": 70,
"cylinder-bandse0.45": 70,
"diabetese0.05": 208,
"diabetese0.07": 231,
"diabetese0.09": 210,
"habermane0.02": 59,
...
def fit_Pure_Search_warm(depth, X, y):
    attack_model = [args.epsilon] * X.shape[1]

    # Exhaustive Search is the same as RobTree with s = $infty$
    
    warm_value = warm_values[args.dataset+"e"+str(args.epsilon)]+1
    ...
    tree = OptimalRobustTreeSearch(
        ...
        s = 1<<30,
        input_file_name=args.dataset + "e" + str(args.epsilon) + "2.txt",
        warm_start_value=warm_value
    )
    tree.fit(X, y)
    optimal_or_not = tree.optimal_
    tree = Model.from_groot(tree)
    return tree, optimal_or_not
def fit_RobTree(depth, X, y):
    attack_model = [args.epsilon] * X.shape[1]
    tree = OptimalRobustTreeSearch(
        attack_model=attack_model,
        time_limit=args.timeout,
        record_progress=False,
        s = 3,
        input_file_name=args.dataset + "e" + str(args.epsilon) + "robtree.txt",
...
def fit_RobTree_warm(depth, X, y):
    attack_model = [args.epsilon] * X.shape[1]
    ...
    tree = OptimalRobustTreeSearch(
        ...
        record_progress=False,
        s = 3,
        input_file_name=args.dataset + "e" + str(args.epsilon) + "robtreewarm.txt",
        warm_start_value=warm_value
    )
...
def fit_Pure_Search(depth, X, y):
    ...
    tree = OptimalRobustTreeSearch(
        ...
        record_progress=False,
        s = 1<<30,
        input_file_name=args.dataset + "e" + str(args.epsilon) + ".txt",
        warm_start_value=1<<30
    )
    tree.fit(X, y)
    optimal_or_not = tree.optimal_
...
if args.fix_depth is None:
    ...
    for depth in range(args.min_depth, args.max_depth + 1):
        ...
        for train_index, test_index in skf.split(X_train, y_train):
            ...
            elif args.algorithm == "Pure-Search":
                model, optimal = fit_Pure_Search(depth, X_train_cv, y_train_cv)
            elif args.algorithm == "Pure-Search-warm":
                model, optimal = fit_Pure_Search_warm(depth, X_train_cv, y_train_cv)
            elif args.algorithm == "RobTree":
                model, optimal = fit_RobTree(depth, X_train_cv, y_train_cv)
            elif args.algorithm == "RobTree-warm":
                model, optimal = fit_RobTree_warm(depth, X_train_cv, y_train_cv)
...
if args.algorithm == "tree":
    ...
    model, optimal = fit_Pure_Search(best_depth, X_train, y_train)
elif args.algorithm == "Pure-Search-warm":
    model, optimal = fit_Pure_Search_warm(best_depth, X_train, y_train)
elif args.algorithm == "RobTree":
    model, optimal = fit_RobTree(best_depth, X_train, y_train)
elif args.algorithm == "RobTree-warm":
    model, optimal = fit_RobTree_warm(best_depth, X_train, y_train)
...
if args.fix_depth is None:
    ...
    results["time_taken"] = time_1- time_0
else:
